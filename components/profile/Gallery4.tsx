...
export default function Gallery4({ address, tokens }: Gallery4Props) {
            ...
            <div className="grid grid-cols-1 col-span-3 gap-2 md:gap-3">
                <div className="w-full grid grid-cols-3 gap-2 md:gap-3">
                    <div className="grid col-span-2">
                        <ImageGalleryProfile address={address} token={mainToken} size="md" />
                    </div>
                    <div className="grid grid-cols-1 gap-2 md:gap-3">
                        {/* Stack tokens with empty slot validation */}
                        {stackTokens.map((token, index) => (
                            <ImageGalleryProfile address={address} key={token?.id || index} token={token} size="sm" />
                        ))}
                    </div>
                </div>
            </div>
