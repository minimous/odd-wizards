...
export default function Profile({ params }: { params: { address: string } }) {
    ...
    const postToTwitter = (tweetData: { text: string, image: string }) => {
        try {
            // Encode the text for URL
            const encodedText = encodeURIComponent(tweetData.text);
            
            // Encode the image URL
            const encodedImage = encodeURIComponent(tweetData.image);
    ...
    const doTweet = async () => {
        try {
            // Generate the image
            const imageUrl: string | undefined = await exportToImage();
    ...
    const exportToImage = async (): Promise<string | undefined> => {
