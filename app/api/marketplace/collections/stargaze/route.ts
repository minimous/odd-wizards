...
function formatPriceInfo(priceData: any): FormattedPriceInfo {
    ...
    const value = parseFloat(priceData.amount) / Math.pow(10, priceData.exponent || 6);
...
function formatCollection(rawCollection: any): FormattedCollection {
    ...
    const forSalePercentage = activeTokens > 0 ? Math.round((listedTokens / activeTokens) * 100) : 0;
    ...
    return {
        ...
        imageUrl: rawCollection.media?.visualAssets?.md?.url || rawCollection.media?.url || '',
    ...
    try {
        ...
        const params: MarketplaceCollectionParams = {
            ...
            filterByCategories: filterByCategories ?
                (Array.isArray(filterByCategories) ? filterByCategories : [filterByCategories]) as string[] :
                undefined,
            minMaxFilters: minMaxFilters ? JSON.parse(minMaxFilters as string) : {},
            filterByDenoms: filterByDenoms ?
                (Array.isArray(filterByDenoms) ? filterByDenoms : [filterByDenoms]) as string[] :
                undefined,
        ...
        const pageInfo = {
            ...
            hasNextPage: (rawResponse.pageInfo.offset + rawResponse.pageInfo.limit) < rawResponse.pageInfo.total,
